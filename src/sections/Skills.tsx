        ...
        <section id="skills" className="py-32 relative bg-[#0a0a0a]">
            ...
            <div className="relative z-10 max-w-5xl mx-auto px-6">
                ...
                <div className="flex justify-center mb-12">
                    <div className="flex p-1 bg-white/5 rounded-full backdrop-blur-sm border border-white/5">
                        {allTabs.map((tab) => (
                            <button
                                key={tab}
...
const SkillCard = ({ skill }: { skill: { name: string, icon: string } }) => {
    ...
    return (
        <motion.div
            whileHover={{ y: -5 }}
            className="flex flex-col items-center p-6 bg-white/5 border border-white/5 rounded-xl hover:bg-white/10 hover:border-primary/30 transition-all duration-300 group"
        >
            <img
                src={iconUrl}
                alt={skill.name}
                className="w-10 h-10 mb-4 invert opacity-70 group-hover:opacity-100 transition-opacity"
            />
